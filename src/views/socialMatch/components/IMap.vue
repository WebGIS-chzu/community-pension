...
export default {
  ...
  methods: {
    ...
    async getRecommendElderlyByCondition(value) {
      var resemble =parseFloat(value.value2) / 100
      console.log(value.id, value.checkedTimes[0], value.value1, resemble)
      await getRecommendElderlyByCondition({"userid": value.id, 
        "dateRange": value.checkedTimes[0], 
        "radius": value.value1,
        "resemble": resemble}).then(res => {
        if (res) {
          console.log(value.id, value.checkedTimes[0], value.value1, resemble)
          console.log("res", res)
          bus.$emit("condition", res)
        }
      })
    },
    getElderlyLocation() {
    ...
    showPosition() {
      ...
      if (this.locallist.length) {
        ...
        that.map.on('click', 'old', async function(e) {
          // 获取用户id从而得到具体用户信息
          that.id = ""
          that.id = e.features[0].properties.description;
          console.log("that.id", that.id)
          bus.$on("sendValue", function(value) {
            that.removePopup();
            if (that.id) {
              value.id = that.id
              that.condition = value
              that.getRecommendElderlyByCondition(value)
            }else {
              alert("请先选择所要查看的老人！")
            }
          })
          var coordinates = e.features[0].geometry.coordinates.slice();
          await getUserInformationByUserId({"userid": that.id}).then(function(res) {
            if(res) {
        ...
        that.map.on('mouseenter', 'old', function(e) {
          that.map.getCanvas().style.cursor = 'pointer';
          // Ensure that if the map is zoomed out such that multiple
          // copies of the feature are visible, the popup appears
          // over the copy being pointed to.
          var id1 = e.features[0].properties.description
